...
Я владею Python, SQL, основами статистики, визуализацией и дашбордами. Осваиваю машинное обучение и применяю это в проектах. Особенно горжусь своей работой по анализу новостных текстов — это был мой первый опыт в NLP, и он зацепил меня всерьёз.
...
[Проекты](#проекты)
  - Python
    ...
    - [A/B тест для мобильной игры Cookie Cats](#ab-тест-для-мобильной-игры-cookie-cats)
  - SQL
    - [Исследование продуктовых метрик](#исследование_продуктовых_метрик)
    - [Сертификаты](#сертификаты-sql)
  - Дашборды
...
### A/B тест для мобильной игры Cookie Cats
**Код:**[A/B тест для мобильной игры Cookie Cats](https://github.com/maleevaan/Portfolio-projects/blob/main/cookie_cats.ipynb)

**Цель:** Понять, как повлияет перенос progression gate (барьера для продвижения игроков) с уровня 30 на уровень 40 на ключевые продуктовые метрики.

**Описание:** Проведён EDA: распределение сессий, выявление аномалий, сравнение групп. Проверены метрики удержания: D1 и D7. Для D1 и D7 выполнен z-тест для пропорций. Для количества сыгранных раундов использован тест Манна–Уитни, так как распределения смещённые и тяжёлые. Для каждой метрики рассчитан MDE, который можно зафиксировать при размерах выборок. Сформулированы продуктовые выводы о корректности дизайна теста.

**Навыки:** EDA: анализ распределений, выбросов и особенностей данных, A/B-тестирование в игровой аналитике, проверка статистических гипотез (z-test пропорций, Mann–Whitney U), ретроспективный расчёт MDE, работа с retention-метриками, формирование продуктовых выводов и оценка качества дизайна эксперимента

**Технологии:** Python, Pandas, NumPy, Matplotlib, Seaborn, SciPy, statsmodels

**Результаты:** Retention D1: статистически значимое отличие, но эффект минимальный и близок к MDE → нет реальной продуктовой ценности. Retention D7: различий нет. Сыгранные раунды: распределения сходны, тест Манна–Уитни показывает отсутствие различий. Вывод: перенос progression gate не улучшает ключевые метрики. Для надёжного результата тест следует перезапустить с корректным дизайном, заранее рассчитанным MDE и чётким набором целевых метрик.
